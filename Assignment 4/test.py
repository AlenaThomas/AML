...
class TestScore(unittest.TestCase):
    ...
    def test_spam(self):
        spam_text = "Claim your free home depot gift card - a $ 1000 value!"
...
class TestFlaskIntegration(unittest.TestCase):
    ...
    def test_docker(self):
        ...
        container_id = subprocess.check_output(['docker', 'run', '-d', '-p', '5000:5000', 'flask-app']).decode('utf-8').strip()
        time.sleep(5)

        try:
            text = "Congratulations! You have won a vacation to the Bahamas! Click here to win the prize!"
            response = requests.post('http://localhost:5000/score', json={'Text': text})
